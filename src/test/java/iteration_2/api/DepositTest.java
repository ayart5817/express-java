...
public class DepositTest extends BaseTest {
    ...
    public void user1GenerateTest() {
        ...
        account1 = new ValidatedCrudRequester<AccountResponse>(RequestSpecs.authAsUser(user1.getUsername(), password), Endpoint.ACCOUNTS, ResponseSpecs.entityWasCreated()).postAndExtract(null);
    ...
    void verificationDefaultU1() {
        UserProfileResponse profile = ProfileSteps.getProfile(user1.getUsername(), password);
    ...
    public void user2GenerateTest() {
        ...
        account2 = new ValidatedCrudRequester<AccountResponse>(RequestSpecs.authAsUser(user2.getUsername(), password2), Endpoint.ACCOUNTS, ResponseSpecs.entityWasCreated()).postAndExtract(null);
    ...
    void verificationDefaultU2() {
        UserProfileResponse profile = ProfileSteps.getProfile(user2.getUsername(), password2);
    ...
    void depositBelowLimit() {
        ...
        AccountResponse response = UserSteps.makeDeposit(user2.getUsername(), password2, account2.getId(), amount);
    ...
    void verificationTransactionU1AfterFirstDeposit() {
        List<Transaction> transactions = UserSteps.getTransactions(user2.getUsername(), password, account2.getId());

        List<Transaction> sorted = transactions.stream().sorted(Comparator.comparing(Transaction::getId)).collect(Collectors.toList());
    ...
    void verificationTransactionU1AfterDepositExactLimit() {
        List<Transaction> transactions = UserSteps.getTransactions(user2.getUsername(), password2, account2.getId());
    ...
    void depositAboveLimit() {
        String error = UserSteps.makeDepositFails(user2.getUsername(), password2, account2.getId(), 5000.01);
    ...
    void depositZero() {
        double amount = 0.00;
        String error = UserSteps.makeDepositFails(user2.getUsername(), password2, account2.getId(), amount);
    ...
    void depositToForeignAccount() {
        ...
        String error = UserSteps.makeDepositForbidden(user1.getUsername(), user1.getPassword(), account2.getId(), amount);
    ...
    void depositMinLimit() {
        double amount = 0.01;
        AccountResponse response = UserSteps.makeDeposit(user2.getUsername(), password2, account2.getId(), 0.01);
    ...
    void verificationBalanceU1() {
        List<Transaction> transactions = UserSteps.getTransactions(user1.getUsername(), user1.getPassword(), account1.getId());
